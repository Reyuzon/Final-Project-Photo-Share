        ...
        {isLoading
          ...
          : data.pages.map((page) =>
              page.data.data.posts.map((post, i) => (
                <Link className="relative group" href={`?p=${post.id}`}>
                  <Img
                    className="w-full rounded-3xl mb-4"
                    key={i}
                    src={post.imageUrl}
                    fallback={
                      <div class="flex items-center justify-center w-full h-48 rounded-3xl mb-4 bg-gray-300 dark:bg-gray-700">
                        <svg
                  ...
                  />
                  <div className="absolute rounded-3xl bg-black/50 w-full h-full top-0 right-0 items-center justify-center gap-3 text-white hidden group-hover:flex">
